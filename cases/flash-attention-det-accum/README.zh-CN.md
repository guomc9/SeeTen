...
| `out/v3-index-schedules.pptx` | 生成的页面（32 页：总览 + 轴遍历与 dS 分块 + 虚拟列专页 + 7 种方法 x 2 + 一页更难的 S2 ≠ S1 例子 + 十页性能对比：确定性开销 / det-vs-det / nd-vs-nd 各按 小/中/大 shape 分页（横轴 = case 序号 + 数据 size MB；每页 BSND｜TND 两幅全宽图 + 两张按 case 的 shape/核时表，含 GM/pass 汇总行）+ 全矩阵明细 BSND/TND 两页 + profiling 三页：分 pipe 用时表（BSND 小/中/大、TND causal 小/中/大）+ 瓶颈结论与优化优先级） |
