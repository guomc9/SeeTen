...
- **有性能对比数据时**：核时表（profiling 时间）与吞吐/带宽柱状图 —— 可选，没数据不画。
...
2. **不留大块空白。** 留白处要补**有用的**表，不是用废话填空。可选补法：
   ...
   后的那张表）、对象说明（每个核心负责哪些列）、转置视角（`sd.trace_table`：行 = 核、
   列 = 轮，答"一条核连着几轮守同一列"）。
   一页分两栏时**别让某一栏只填到一半**：把辅助表按优先级塞进两栏（高度先用
   `sd.panel_height` 算好，放不下的宁可少一张表，也不要压到已画好的元素上；
   配方见 `references/diagram-recipes.md` 配方 9）。
...
9. **配色用预设色组。** 从 `assets/color-sets.json` 的 `lane_sets` 选一套：
   ...
   `plain`（**没有语义要编码**时用：只放序号的网格，或要圈出某一行/列的图）。
   每色自带文字色，避免浅底压白字；空位用 `hole` 灰。
10. **解释文字放表格外面，而且越短越好。** 一行一句，能删的词就删；表格区域只放表格。
    重点用**行内强调**顶上去，不要靠加长句子：`sd.text(...)` 和格内文本都支持
    `**加粗**` 与 `==换成强调色==`（标记本身不会画出来）—— 一页里强调两三处就够。
...
12. **标题带序号**（`1.` `2.` …），并按页序组织；每条规则要**标清适用场景**：
    用 `sd.tag_row(...)` 打一行标签（layout / causal / MHA / GQA / 是否列私有 之类），
    标签用常用英文缩写，不自造中文名；单值标签写空串标签（`("", "MHA")`）；
    值用肯定/否定短语（`非 causal`），不要写 `是/否` —— 标签加值连读成 `causal 否` 很别扭；
    别让读者自己猜这条规则适用于什么形状。
...
14. **颜色别只做一级区分。** 只按一个维度上色时，同一组的东西长得一模一样，分不出先后 ——
    再叠一层同一种颜色的深浅（`sd.lane_shades`：颜色 = 哪一组、同一个颜色的深浅 = 组内第几个，
    例如颜色 = batch，深浅 = 第几条 KV 列）。别再加图例色块解释颜色，把颜色规则写进副标题一句话即可。
15. **写清规则在本实现里的地位。** 画"算法全集 / 移植"类图时，每条规则按可达性标注：
