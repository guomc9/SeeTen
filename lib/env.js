...
/** 解释器能不能 import 某个可选模块（如 matplotlib）。 */
function hasModule(py, args = [], name) {
  return run(py, [...args, '-c', `import ${name},sys;sys.stdout.write("ok")`]).code === 0;
}
...
function ensurePython({ quiet = false } = {}) {
  ...
  const pip = run(vp, ['-m', 'pip', 'install', '--quiet', '--disable-pip-version-check',
    '--upgrade', 'python-pptx', 'matplotlib']);
  if (pip.code !== 0) {
    bad('安装 python-pptx / matplotlib 失败：' + (pip.err || pip.out));
    return null;
  }
  if (!quiet) ok(`已建好独立环境并安装 python-pptx / matplotlib → ${VENV_DIR}`);
  return { cmd: vp, args: [], source: 'venv' };
}
...
module.exports = {
  ...
  findPython, ensurePython, hasPptx, hasModule, venvPython, findRenderer,
